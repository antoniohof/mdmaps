...
function drawKeypoints(keypoints) {
  ...
  let dist = distanceBetween(dedaoP, indexP)
  let now = Date.now()
  if (dist < 70 && (now - lastBeat) > 250) {
    let time = Date.now()
    lastBeat = time
    console.log('beat@')
    addBeat(time)
    ...
    if (predictions.length > 0) {
      ...
      if (renderPointcloud === true && scatterGL != null) {
        ...
        if (!scatterGLHasInitialized) {
          ...
          mappedFingers = Object.keys(fingerLookupIndices);

          scatterGL.setSequences(
            mappedFingers.map(finger => ({indices: fingerLookupIndices[finger]})));
